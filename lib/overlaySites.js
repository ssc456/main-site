...
function shouldUseDemoStore() {
  return isExplicitDemoMode() || !redis;
}

const DEFAULT_MONTHLY_PRICE_LABEL = '14.99';
const DEFAULT_YEARLY_PRICE_LABEL = '149.99';
const LEGACY_MONTHLY_PRICE_LABELS = new Set(['25']);
const LEGACY_YEARLY_PRICE_LABELS = new Set(['250']);

function normalizePriceLabel(value, defaultValue, legacyDefaults) {
  const label = String(value ?? '').trim();

  if (!label || legacyDefaults.has(label)) {
    return defaultValue;
  }
...
export function getOverlayPriceId(interval = 'monthly') {
  if (interval === 'yearly') {
...
export function hasLiveBillingPlan(interval = 'monthly') {
  return Boolean(process.env.STRIPE_SECRET_KEY?.trim() && getOverlayPriceId(interval));
}

export function hasAnyLiveBillingPlan() {
  return hasLiveBillingPlan('monthly') || hasLiveBillingPlan('yearly');
}
...
function normalizeOverlaySite(site = {}) {
  return {
    ...
    monthlyPriceLabel: normalizePriceLabel(site.monthlyPriceLabel, DEFAULT_MONTHLY_PRICE_LABEL, LEGACY_MONTHLY_PRICE_LABELS),
    yearlyPriceLabel: normalizePriceLabel(site.yearlyPriceLabel, DEFAULT_YEARLY_PRICE_LABEL, LEGACY_YEARLY_PRICE_LABELS),
...
export function isDemoBillingMode() {
  return isExplicitDemoMode() || !hasAnyLiveBillingPlan();
}
